...
import { classFromData, FieldSet } from "../src/oo.js";
import { expectFieldRegistered } from "./dom.js";

describe("FieldSet", () => {
  test("is registered in fieldsByTypeName", () => {
    expectFieldRegistered("set", FieldSet);
  });

  test("validates ISet values", () => {
  ...
  test("classFromData detects ISet", () => {
  ...
  test("proto: deleteInTags / removeInTags", () => {
    ...
    expect(inst.removeInTags(3).get("tags").has(3)).toBe(false);
  });
  ...
  test("proto: resetTags", () => {
    ...
    const r = Cls().addInTags("a").addInTags("b").resetTags();
  ...
  test("proto: toggleInTags adds value when not present", () => {
    ...
    const r = Cls().toggleInTags("foo");
    expect(r.hasInTags("foo")).toBe(true);
  });

  test("proto: toggleInTags removes value when present", () => {
    const Cls = classFromData("SetToggle2", { fields: { tags: ISet() } });
    const inst = Cls().addInTags("foo");
    expect(inst.hasInTags("foo")).toBe(true);
    expect(inst.toggleInTags("foo").hasInTags("foo")).toBe(false);
  });

  test("proto: toggleInTags round trip", () => {
    const Cls = classFromData("SetToggle3", { fields: { tags: ISet() } });
    const step1 = Cls().toggleInTags("bar");
    expect(step1.hasInTags("bar")).toBe(true);
    expect(step1.toggleInTags("bar").hasInTags("bar")).toBe(false);
  });
});
