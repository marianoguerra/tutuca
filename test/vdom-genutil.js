...
export function randString(rng, maxLen = 10, options = {}) {
    ...
    if (includeSpaces)
        chars += "   "; // Add extra spaces to increase probability
    if (includeNewlines)
        chars += "\n\n"; // Add extra newlines
...
export function getNodeAtPath(root, path) {
    ...
    for (const idx of path) {
        if (!current.childs || idx >= current.childs.length)
            return null;
...
export function generateTree(rng, depth = 0, options = {}) {
    const { maxDepth = 3, includeSpacesInText = true, includeNewlinesInText = true, includeSpacesInClassName = true, } = options;
    ...
    if (isLeaf) {
        ...
        return h(tag, props, randString(rng, 10, {
            includeSpaces: includeSpacesInText,
            includeNewlines: includeNewlinesInText,
        }));
    }
...
export function generateMutation(rng, tree, options = {}) {
    const { includeSpacesInText = true, includeNewlinesInText = true, includeSpacesInClassName = true, } = options;
...
export function applyMutation(root, mutation, rng) {
    ...
    if (!parent || !(parent instanceof VNode))
        return clone;
    switch (mutation.type) {
        ...
        case "changeTag": {
            if (mutation.path.length === 0) {
                return new VNode(mutation.newTag.toUpperCase(), clone.attrs, clone.childs, clone.key, clone.namespace);
            }
            ...
            if (parentNode instanceof VNode && parentNode.childs[targetIdx] instanceof VNode) {
                ...
                parentNode.childs[targetIdx] = new VNode(mutation.newTag.toUpperCase(), old.attrs, old.childs, old.key, old.namespace);
            }
            break;
        }
        case "addChild": {
        ...
        case "changeStyle": {
            const existing = typeof parent.attrs.style === "string"
                ? cssToStyleObj(parent.attrs.style)
                : {};
