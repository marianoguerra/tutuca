import { component, css, html } from "tutuca";

const JsonNull = component({
  name: "JsonNull",
  view: html`<span class="text-warning text-xs font-mono p-3">null</span>`,
});

const JsonBool = component({
  receive: {
    toggleValue(draft) {
      draft.value = !draft.value;
    },
  },
  name: "JsonBool",
  fields: { value: true },
  view: html`<button
    @if.class=".value"
    @then="'btn btn-sm btn-soft btn-success font-mono'"
    @else="'btn btn-sm btn-soft btn-error font-mono'"
    @on.click="toggleValue"
  >
    <span @show=".value">true</span>
    <span @hide=".value">false</span>
  </button>`,
});

const JsonString = component({
  receive: {
    setValue(draft, value) {
      draft.value = value;
    },
  },
  name: "JsonString",
  fields: {
    value: "hello",
  },
  view: html`<input
    class="input input-sm input-ghost text-green-500 font-mono"
    :value=".value"
    @on.input="setValue e.value"
  />`,
});

const JsonNumber = component({
  name: "JsonNumber",
  fields: {
    strValue: "0",
    value: 0,
  },
  receive: {
    setRawValue(draft, v) {
      const value = parseFloat(v);
      draft.strValue = v;
      if (Number.isFinite(value)) draft.value = value;
    },
  },
  view: html`<input
    type="number"
    class="input input-sm input-ghost text-primary font-mono"
    :title=".value"
    :value=".strValue"
    @on.input="setRawValue e.value"
  />`,
});

export const JsonSelector = component({
  name: "JsonSelector",
  // The types this selector can build. One handler still serves every button: the
  // view names the type and the handler resolves it, which is what the declaration
  // list makes checkable.
  lookup: ["JsonNull", "JsonBool", "JsonNumber", "JsonString", "JsonArray", "JsonObject"],
  receive: {
    setTo(_draft, name, ctx) {
      return ctx.lookupType(name).make();
    },
    setToBool(_draft, isCtrl, ctx) {
      return ctx.lookupType("JsonBool").make({ value: !isCtrl });
    },
  },
  view: html`<div class="join gap-3 font-mono">
    <button class="btn btn-sm btn-soft btn-warning" @on.click="setTo 'JsonNull'">null</button>
    <button class="btn btn-sm btn-soft btn-success" @on.click="setToBool e.isCtrl">
      bool
    </button>
    <button class="btn btn-sm btn-soft btn-primary" @on.click="setTo 'JsonNumber'">0</button>
    <button class="btn btn-sm btn-soft btn-accent" @on.click="setTo 'JsonString'">""</button>
    <button class="btn btn-sm btn-soft btn-info" @on.click="setTo 'JsonArray'">[]</button>
    <button class="btn btn-sm btn-soft btn-info" @on.click="setTo 'JsonObject'">{}</button>
  </div>`,
});

const JsonArray = component({
  name: "JsonArray",
  fields: {
    items: [],
  },
  receive: {
    removeInItemsAt(draft, key) {
      draft.items.splice(key, 1);
    },

    addItem(draft, ctx) {
      draft.items.push(ctx.lookupType("JsonSelector").make());
    },
    onDropOnItem(draft, key, info) {
      draft.items = this.items.moveKeyBeforeKey(info.lookupBind("key"), key);
    },
  },
  style: css`
    [data-draggingover="json-list-item"] {
      border-top: 4rem solid transparent;
      transition: 0.15s border-top ease;
    }
  `,
  view: html`<section class="join join-vertical gap-3 pl-3 border-l-1 border-l-neutral-500">
    <button class="btn btn-sm btn-soft btn-primary" @on.click="addItem">+</button>
    <div class="join join-vertical gap-3">
      <div
        class="join gap-3 justify-between items-center group"
        @each=".items"
        draggable="true"
        data-dragtype="json-list-item"
        data-droptarget="json-list-item"
        @on.drop="onDropOnItem @key e.dragInfo"
      >
        <x render-it></x
        ><button
          class="btn btn-sm btn-soft btn-circle btn-error font-mono opacity-20 group-hover:opacity-100"
          @on.click="removeInItemsAt @key"
        >
          x
        </button>
      </div>
    </div>
  </section>`,
});

const JsonObjectKeyVal = component({
  receive: {
    setKey(draft, value) {
      draft.key = value;
    },
  },
  name: "JsonObjectKeyVal",
  fields: {
    key: "k",
    value: null,
  },
  view: html`<div class="join gap-3 items-center justify-center">
    <input
      class="input input-sm input-ghost text-green-500 font-mono"
      :value=".key"
      @on.input="setKey e.value"
    />
    <span class="text-xs font-mono text-gray-500">:</span>
    <x render=".value"></x>
  </div>`,
});

const JsonObject = component({
  name: "JsonObject",
  fields: {
    items: [],
  },
  receive: {
    removeInItemsAt(draft, key) {
      draft.items.splice(key, 1);
    },

    addItem(draft, ctx) {
      const KV = ctx.lookupType("JsonObjectKeyVal");
      draft.items.push(KV.make({ value: ctx.lookupType("JsonSelector").make() }));
    },
    onDropOnItem(draft, key, info) {
      draft.items = this.items.moveKeyBeforeKey(info.lookupBind("key"), key);
    },
  },
  style: css`
    [data-draggingover="json-kv-item"] {
      border-top: 4rem solid transparent;
      transition: 0.15s border-top ease;
    }
  `,
  view: html`<section class="join join-vertical gap-3 pl-3 border-l-1 border-l-neutral-500">
    <button
      class="btn btn-sm btn-soft font-mono btn-primary"
      @on.click="addItem"
    >
      +
    </button>
    <div class="join join-vertical gap-3">
      <div
        class="join gap-3 justify-between items-center group"
        @each=".items"
        draggable="true"
        data-dragtype="json-kv-item"
        data-droptarget="json-kv-item"
        @on.drop="onDropOnItem @key e.dragInfo"
      >
        <x render-it></x
        ><button
          class="btn btn-sm btn-soft btn-circle btn-error font-mono opacity-20 group-hover:opacity-100"
          @on.click="removeInItemsAt @key"
        >
          x
        </button>
      </div>
    </div>
  </section>`,
});

export function getComponents() {
  return [
    JsonSelector,
    JsonNull,
    JsonBool,
    JsonString,
    JsonNumber,
    JsonArray,
    JsonObjectKeyVal,
    JsonObject,
  ];
}

export function getRoot() {
  return JsonArray.make({ items: [JsonSelector.make()] });
}

export function getExamples() {
  return {
    title: "JSON Editor",
    description: "Composable JSON editor built from typed primitives",
    items: [
      {
        title: "Empty Array With Selector",
        description: "Default root: array containing one selector",
        value: getRoot(),
      },
      {
        title: "Object",
        description: "Empty JSON object editor",
        value: JsonObject.make(),
      },
      {
        title: "Type Selector",
        description: "Buttons to choose a JSON value type",
        value: JsonSelector.make(),
      },
      {
        title: "String Value",
        description: "Editable string primitive",
        value: JsonString.make({ value: "tutuca" }),
      },
      {
        title: "Boolean True",
        description: "Boolean toggle in true state",
        value: JsonBool.make({ value: true }),
      },
      {
        title: "Number",
        description: "Editable number primitive",
        value: JsonNumber.make({ value: 42, strValue: "42" }),
      },
    ],
  };
}
